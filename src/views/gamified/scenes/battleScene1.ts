...
export class BattleScene extends Phaser.Scene {
    ...
    // Unified Ground Level from screenshot analysis
    private readonly MASTER_GROUND_Y = 580; // The visual line
    private readonly GROUND_HEIGHT = 60;
    // Physics body is centered. To have TOP at 580, Center = 580 + 30 = 610.
    private readonly PHYSICS_GROUND_Y = 580 + 30; // 610

    // Spawn Y: High enough to fall onto ground.
    private readonly SPAWN_Y = 400;
    ...
    private setupLevel() {
        ...
        // Ground Logic - Single Unified Ground
        if (this.ground) this.ground.destroy();

        // Invisible Physics Ground centered at PHYSICS_GROUND_Y
        this.ground = this.add.rectangle(640, this.PHYSICS_GROUND_Y, 1280, this.GROUND_HEIGHT, 0x000000, 0);
    ...
    private renderDebug() {
        ...
        if (this.game.loop.frame % 60 === 0) {
            ...
            if (this.player) console.log(`Player Y: ${Math.round(this.player.y)} | Ground: ${this.MASTER_GROUND_Y}`);
            this.enemies?.children.iterate((child) => {
                const enemy = child as Enemy;
                console.log(`Enemy ${enemy.id} Y: ${Math.round(enemy.y)} | Ground: ${this.MASTER_GROUND_Y}`);
                return true;
            });
        }
    }
